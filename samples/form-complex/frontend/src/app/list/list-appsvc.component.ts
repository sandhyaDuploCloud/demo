...
import { AppSvcService, AppService, REMOTE_UserSession } from '../appsvc.service';
import { StatusBadgeComponent } from '../shared/status-badge.component';

// List on the platform UI library's <searchable-datatable>. No-provision → no "Track Provisioning" action.
//
  ...
  template: `
    <div class="card datatable-card">
      <searchable-datatable
        [showAdd]="true" addLabel="Create App Service" (add)="add()" [rows]="rows()" (filter)="filterUpdate()" columnMode="force">
        ...
        <ngx-datatable-column name="Image" [flexGrow]="180">
          <ng-template ngx-datatable-cell-template let-row="row">{{ row.spec?.image || '—' }}</ng-template>
        </ngx-datatable-column>

        <ngx-datatable-column name="Platform" [flexGrow]="90">
          <ng-template ngx-datatable-cell-template let-row="row">{{ row.spec?.platform || '—' }}</ng-template>
        </ngx-datatable-column>
...
export class ListAppSvcComponent implements OnInit {
  ...
  protected view(r: AppService): void {
